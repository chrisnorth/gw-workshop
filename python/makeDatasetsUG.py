...
for d in datasets:
    ...
    tab=Table.read(os.path.join(dir,file))
    tab.remove_column('hc')
    tab.rename_column('t','time (s)')
    tab.rename_column('hp','strain')
    tab.write('Dataset{}_nonoise.fits'.format(d),overwrite=True)
    tab.write('Dataset{}_nonoise.csv'.format(d),format='ascii.csv',overwrite=True)
    noise24=(random.random(len(tab))-0.5)*1e-24
    tab24=Table([tab['time (s)'],tab['strain']])
    tab24['strain']=tab24['strain']+noise24
    tab24.write('Dataset{}_noise24.fits'.format(d),overwrite=True)
    tab24.write('Dataset{}_noise24.csv'.format(d),format='ascii.csv',overwrite=True)
    tab23=Table([tab['time (s)'],tab['strain']])

    noise23=(random.random(len(tab))-0.5)*1e-23
    tab23['strain']=tab23['strain']+noise23
    tab23.write('Dataset{}_noise23.fits'.format(d),overwrite=True)
    tab23.write('Dataset{}_noise23.csv'.format(d),format='ascii.csv',overwrite=True)

    noise22=(random.random(len(tab))-0.5)*1e-22
    tab22=Table([tab['time (s)'],tab['strain']])
    tab22['strain']=tab22['strain']+noise22
    tab22.write('Dataset{}_noise22.fits'.format(d),overwrite=True)
    tab22.write('Dataset{}_noise22.csv'.format(d),format='ascii.csv',overwrite=True)
